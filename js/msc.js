...
var newAddMicroserviceModule = (function (Utils) {
    ...
    return function (mscEndpoint, microserviceId, operationId, forceStartWhenStopped, preMenuName, preInputs, preAlg) {
        ...
        var _dom = {
        	rootNode: $('<div>'),
            messageDiv: $('<div>'),
            serviceNameTxt: $('<input type="text" class="form-control" placeholder="Unique Name">').val(preMenuName?preMenuName:''),
            inputs: {},
            ...
            testCallBtn: $('<button class="btn btn-primary" type="button">Test a Call</button>').click(function () {
                var _processResult = function (dataCall) {
                    _dom.resultTxt.val(JSON.stringify(dataCall, null, 4));
                    var alg = aceEditor.getValue();
                    ...
                    try {
                        ...
                        var domDemoRes = algF(dataCall);
                        _dom.resultDemoDiv.empty().append(domDemoRes);
                    } catch (e) {
                        Utils.showError(e, _dom.rootNode);
                    }
                };
                Object.keys(requiredInputs).forEach(function (inputId) {
                    requiredInputs[inputId].value = _dom.inputs[inputId].txt.val();
                });
                _dom.callInputJsonPre.html(JSON.stringify(requiredInputs, null, 4));
                if(forceStartWhenStopped) {
                    _statics.callMicroserviceForced(mscEndpoint, microserviceId, operationId, requiredInputs, function(dataCall){
                        _processResult(dataCall);
                    }, function(error){
                        Utils.showError(error, _dom.rootNode);
                    });
                } else {
                    _statics.callMicroservice(mscEndpoint, microserviceId, operationId, requiredInputs, function(dataCall){
                        _processResult(dataCall);
                    }, function(error){
                        Utils.showError(error, _dom.rootNode);
                    });
        ...
        var _fns = {
            getMsInputJson: function () {
                Object.keys(requiredInputs).forEach(function (inputId) {
                    requiredInputs[inputId].value = _dom.inputs[inputId].txt.val();
                });
                return requiredInputs;
            },
            loadMsInputs: function () {
                _statics.getMicroserviceIOInfo(mscEndpoint, microserviceId, operationId, function (msIOInfo) {
                    Object.keys(msIOInfo.requiredInputTemplate).forEach(function (inputId) {
                        requiredInputs[inputId] = {value : ''};
                        var inputInfos = msIOInfo.requiredInputTemplate[inputId];
                        _dom.inputs[inputId] = {
                            span: $('<span class="input-group-addon">' + inputId + '</span>').popover({
                                placement : 'auto left',
                            ...
                            txt: $('<textarea style="resize:vertical;" rows="1" class="form-control" placeholder="'+inputInfos.workingExample+'">'+inputInfos.workingExample+'</textarea>')
                        };
                        
                        if(preInputs && preInputs[inputId])
                    	    _dom.inputs[inputId].txt.val(preInputs[inputId].value);
                            ...
                            $('<tr>').append(
        	                	$('<td>').append(
	                	        	$('<div class="input-group">').append(
                	        	        _dom.inputs[inputId].span
	                	        	).append(
                	        	        _dom.inputs[inputId].txt
	                	        	)
        	                	)
                	        )
                        );
            ...
            loadACEEditor: function () {
                aceEditor = ace.edit(_dom.microserviceOutputAdaptAlgDiv[0], {
                    mode: "ace/mode/javascript",
                    autoScrollEditorIntoView: true,
                    minLines: 5
                });
                if(preAlg != null)
                    aceEditor.setValue(preAlg);
            },
            init: function () {
                _fns.loadMsInputs();
            }
        ...
        return {
            ...
            getInputs: function () {
                return {
                    microserviceInputs: _fns.getMsInputJson(),
                    serviceName: _dom.serviceNameTxt.val(),
                    microserviceOutputAdaptAlg: aceEditor.getValue() //_dom.microserviceOutputAdaptAlgTxt.val()
                };
            },
                    ...
                    $('<div class="container-fluid">').append(
                    	$('<div class="row">').append(
                            ...
                            $('<div class="col-md-6">').append(
                                ...
                                _dom.microserviceOutputAdaptAlgDiv
                            )
                        )
                    )
                ).append(
                        ...
                        $('<div class="col-md-6">').append(
                            $('<div class="panel panel-default">').append(
                            	$('<div class="panel-heading">').append(
                    	        	$('<h4 class="panel-title">Service Output Page Preview</h4>')
                            	)
                            ).append(
...
var newInputTableModule = (function (newAddMicroserviceModule) {
        ...
        return {
			render: function () {
                return $('<div class="input-group">').append(
        	        ...
        	        '<span class="input-group-addon">Microservice ID: </span>'
	        	).append(
        	        _dom.microserviceIdTxt
	        	).append(
        	        '<span class="input-group-addon">Operation ID: </span>'
	        	).append(
        	        _dom.operationIdTxt
	        	).append(
        	        '<span class="input-group-addon">Input: </span>'
	        	).append(
        	        _dom.microserviceInputJSONTxt
	        	).append(
        	        '<span class="input-group-addon">Alg: </span>'
	        	).append(
        	        _dom.microserviceOutputAdaptAlgTxt
	        	).append(
    ...
    return function(mscEndpoint) {
        ...
        var _fns = {
            editRow: function (row) {
                ...
                var addMicroserviceModule = newAddMicroserviceModule(mscEndpoint, rowContent.microserviceId, rowContent.operationId, true, rowContent.menuName, JSON.parse(rowContent.microserviceInputJSON), rowContent.microserviceOutputAdaptAlg);
            	
            	Utils.createDialogBootstrap(addMicroserviceModule.render(), 'Edit microservice details', function () {
            	    return true;
            	}, function () {
            	    var msInputs = addMicroserviceModule.getInputs();
            	    row.setContent({
            	        menuName: msInputs.serviceName,
        	            microserviceId: rowContent.microserviceId,
    	                operationId: rowContent.operationId,
	                    microserviceInputJSON: JSON.stringify(msInputs.microserviceInputs),
                        microserviceOutputAdaptAlg: msInputs.microserviceOutputAdaptAlg
            	    });            	    
            	}, function () {
            	    addMicroserviceModule.afterRender();
            	});
            },
        ...
        return {
            ...
            addRowDialog: function (microserviceId, operationId) {
                var addMicroserviceModule = newAddMicroserviceModule(mscEndpoint, microserviceId, operationId, true, null, null, null);
            	
            	Utils.createDialogBootstrap(addMicroserviceModule.render(), 'Add microservice', function () {
            	    return true;
            	}, function () {
            	    var msInputs = addMicroserviceModule.getInputs();
            	    _fns.addRow(msInputs.serviceName, microserviceId, operationId, JSON.stringify(msInputs.microserviceInputs), msInputs.microserviceOutputAdaptAlg);
            	}, function () {
            	    addMicroserviceModule.afterRender();
            	});
            }
...
var _newMSInputsTableModule = (function () {
    ...
    var _newTableRowMSInputModule = function (removeBtnHandlerFn = function () {}) {
        ...
        return {
            render: function () {
                return $('<div class="input-group">').append(
                    '<span class="input-group-addon">Input ID</span>'
                ).append(
                    _dom.inputIdTxt
                ).append(
                    '<span class="input-group-addon">Matching Name</span>'
                ).append(
                    _dom.matchingNameTxt
                ).append(
                    '<span class="input-group-addon">Description</span>'
                ).append(
                    _dom.descriptionTxt
                ).append(
                    '<span class="input-group-addon">Working Sample</span>'
                ).append(
                    _dom.workingExampleTxt
                ).append(
                    _dom.removeRowBtn
                );
            },
        ...
        return {
            render: function () {
                return $('<div class="panel panel-default">').append(
                    '<div class="panel-heading"><h4 class="panel-title">Call Configuration Inputs</h4></div>'
                ).append(
                    $('<div class="panel-body">').append(
                ...
                Object.keys(templateInputs).forEach(function (inputId) {
                    ...
                    _dom.tableTbody.append(
                        $('<tr>').append(
                            $('<td>').append(
                                $('<div class="input-group">').append(
                                    _dom.inputs[inputId].nameSpan
                                ).append(
                                    _dom.inputs[inputId].inputTxt
                                ).append(
...
var _newMSAsyncInputsTableModule = (function () {
    ...
    var _newTableRowMSAsyncInputModule = function (removeBtnHandlerFn = function () {}) {
        ...
        return {
            render: function () {
                return $('<div class="input-group">').append(
                    '<span class="input-group-addon">Asynchronous Input ID</span>'
                ).append(
                    _dom.responseInputIdTxt
                ).append(
                    '<span class="input-group-addon">Value</span>'
                ).append(
                    _dom.responseInputValueTxt
                ).append(
                    _dom.removeRowBtn
                );
            },
...
var _MSAsyncInputsModule = (function (Utils, _newMSAsyncInputsTableModule) {
    ...
    MSAsyncInputsModule.prototype.render = function () {
        return $('<div class="panel panel-default">').append(
            '<div class="panel-heading"><h4 class="panel-title">Configuration for Management of Asynchronous Responses</h4></div>'
        ).append(
            $('<div class="panel-body">').append(
                $('<div class="row form-group">').append(
                    $('<div class="col-lg-12">').append(
                        $('<div class="input-group">').append(
                            '<span class="input-group-addon">Input Adaptation Algorithm</span>'
                        ).append(
                            this._doms.inputAdaptationAlgorithmTxt
                        )
                    )
                )
            ).append(
                $('<div class="row form-group">').append(
                    $('<div class="col-lg-12">').append(
                        $('<div class="input-group">').append(
                            '<span class="input-group-addon">Responses to Microservice ID</span>'
                        ).append(
                            this._doms.responseServiceIdTxt
                        ).append(
                            '<span class="input-group-addon">using Operation</span>'
                        ).append(
                            this._doms.responseServiceOperationIdTxt
                        ).append(
                            '<span class="input-group-addon">and Input</span>'
                        ).append(
                            this._doms.responseServiceInputIdTxt
                        )
                    )
                )
            ).append(
                $('<div class="row form-group">').append(
                    $('<div class="col-lg-3">').append(
                        this._doms.addAsyncRespMsInputBtn
                    )
                )
            ).append(
                $('<div class="row form-group">').append(
...
var _newMSOperationModule = (function (Utils, _newConnectorConfiguration, _newMSInputsModule, _MSAsyncInputsModule) {
                ...
                _doms.panelCollapsable.append(
                    $('<div class="panel-body">').append(
                        $('<div class="row form-group">').append(
                            $('<div class="col-lg-12">').append(
                                $('<div class="input-group">').append(
                                    ...
                                    _doms.descriptionTxt
                                ).append(
                                    '<span class="input-group-addon">Is Default?</span>'
                                ).append(
                        ...
                        $('<div class="row form-group">').append(
                            $('<div class="col-lg-4">').append(
                                $('<div class="input-group">').append(
                                    ...
                                    _doms.connectorIdSelect
                                )
                            )
                        ).append(
                            $('<div class="col-lg-8">').append(
                                _doms.connectorDescDiv
                            )
                        )
                    ).append(
                        $('<div class="row form-group">').append(
                            $('<div class="col-lg-6">').append(
                                _subs.startConfigModule.render()
                            )
                        ).append(
                            $('<div class="col-lg-6">').append(
                                _subs.callConfigModule.render()
                            )
                        )
                    ).append(
                        $('<div class="row form-group">').append(
                            $('<div class="col-lg-12">').append(
                                _subs.msInputsModule.render()
                            )
                        )
                    ).append(
                        _doms.asyncInputsDiv.append(
                            $('<div class="col-lg-12">').append(
                                _subs.msAsyncInputsModule.render()
                            )
                        ).hide()
                    ).append(
                        $('<div class="row form-group">').append(
                            $('<div class="col-lg-12">').append(
                                $('<div class="input-group">').append(
                                    '<span class="input-group-addon">Connector Output Description</span>'
                                ).append(
                                    _doms.connectorOutputDescTxt
                                )
                            )
                        )
                    ).append(
                        $('<div class="row form-group">').append(
                            $('<div class="col-lg-12">').append(
                                $('<div class="input-group">').append(
                                    '<span class="input-group-addon">Output Description</span>'
                                ).append(
                                    _doms.outputDescriptionTxt
                                )
                            )
                        )
                    ).append(
                        $('<div class="row form-group">').append(
                            $('<div class="col-lg-12">').append(
                                $('<div class="input-group">').append(
                                    '<span class="input-group-addon">Output Adaptation Algorithm</span>'
                                ).append(
                                    _doms.outputAdaptationAlgorithmTxt
                                )
                            )
                        )
                    ).append(
                        $('<div class="row form-group">').append(
                            $('<div class="col-lg-12">').append(
                                $('<div class="input-group">').append(
                                    '<span class="input-group-addon">Status Check Algorithm</span>'
                                ).append(
    ...
    return function (config = {}, removeBtnHandlerFn = function () {}, defaultChkChangeHandlerFn = function () {}) {
        ...
        var _content = {};
        var _subs = {
            startConfigModule: _newConnectorConfiguration({title: 'Start configuration', lang: lang, mscEndpoint: mscEndpoint}),
            callConfigModule: _newConnectorConfiguration({title: 'Call configuration', lang: lang, mscEndpoint: mscEndpoint}),
                        ...
                        $('<div class="col-lg-12">').append(
                            $('<div class="input-group">').append(
                                '<span class="input-group-addon">Image URL</span>'
                                    ...
                                    _doms.presentationImageUrlTxt
                            ).append(
                                $('<span class="input-group-btn">').append(
                                    _doms.imageUploadBtn
                                ).append(
                                    _doms.imageUploadFile
                                )
                            )
                        )
                    )
    ...
    function static_render(_doms, _subs) {
        return $('<div>').append(
            $('<div class="row form-group">').append(
                $('<div class="col-lg-12">').append(
                    $('<div class="input-group">').append(
                        '<span class="input-group-addon">Microservice Name</span>'
                    ).append(
                        _doms.nameTxt
                    ).append(
                        '<span class="input-group-addon">Description</span>'
            ...
            _doms.detailsDiv.append(
                $('<div class="col-lg-12">').append(
                    _subs.msDetailsModule.render()
                )
            )
        ).append(
            $('<div class="row form-group">').append(
                $('<div class="col-lg-3">').append(
                    _doms.addOperationBtn
                )
            )
        ).append(
            _doms.operationsDiv
    ...
    return function(config) {
        ...
        var _dom = {
            ...
            addMicroserviceBtn: $('<button class="btn btn-default" type="button">Add</button>').click(function () {
                try {
                	...
                	if(microserviceId=='' || operationId=='') throw 'Select a microservice and an operation';
        ...
        return {
            render: function () {
                return $('<div>').append(
                	$('<div class="input-group">').append(
        	        	'<span class="input-group-addon">Select a Microservice</span>'
                	).append(
            	        _dom.allMicroserviceSelect
                	).append(
            	        '<span class="input-group-addon"></span>'
        	        ).append(
    	                _dom.allMicroserviceOperationsSelect
        	        ).append(
	                	$('<span class="input-group-btn">').append(
                	        _dom.addMicroserviceBtn
	                	).append(
                	        _dom.newMicroserviceBtn
	                	).append(
                	        _dom.demoMicroserviceBtn
	                	)
        	        )
                ).append('<br>').append(
                    inputTableModule.render()
                ).append('<br>').append(
                    _dom.saveBtn
                ).append(
                    _dom.messageDiv
